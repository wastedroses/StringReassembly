...
public final class StringReassembly2 {
    ...
    public static String combination(String str1, String str2, int overlap) {
         ...
         */
        //declating variables
        //stringIndex is the length of the string subtracted from
        //how much overlap is being looked for. Once that overlap is found,
        //the index is set to where the true/overlapped string is at.
        int stringIndex = str1.length() - overlap;

        //combinationString is the substring starting at 0, and ending at where
        //the overlapped text is at.
        String combinationString = str1.substring(0, stringIndex);

        //The final string is the combinationString and the missing string.
        String finalString = combinationString + str2;
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        //creating variables
        Set<String> replacement = strSet.newInstance();
        boolean runAgain = true;

        //for-while loop that iterates through the strings of the set.
        for (String s : strSet) {
            //conditional that says if the string is inside of the set, then
            //it doesn't have to iterate again.
            if (s.indexOf(str) >= 0) {
                runAgain = false;
            }
        }
        //if the string isn't inside the set, then it iterates.
        if (runAgain == true) {
            //for-while loop that iterates through the strings of the set.
            for (String s : strSet) {
                //if the string isn't in the set, then it's added to a different
                //set (new set made from set in parameters)
                if (str.indexOf(s) < 0) {
                    //adds string to new set
                    replacement.add(s);
                }
            }
            //takes the value from the filler set and transfers it back into the
            //original set. The string is then added to the original set
            //instead of the newInstance set.
            strSet.transferFrom(replacement);
            strSet.add(str);
        }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        //adds input to set
        stringSet.add(firstLine);

        //getting new input

        //if the input/string has ended (nothing left in input), then calls
        //addToSetAvoidingSubstrings method
        while (!input.atEOS()) {
            String in = input.nextLine();
            addToSetAvoidingSubstrings(stringSet, in);
        }
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        //for loop that replaces ~ with a space.
